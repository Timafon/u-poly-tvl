import { getBalance, multiCall, sumMultiBalanceOf } from "./DefilLamaSDK";

const { default: axios } = require("axios");
const BigNumber = require("bignumber.js");

export async function calculatePolyTvl(_, block) {
  ...
  const lockedPoSBalances = await multiCall({
    calls: posTokens,
    abi: "erc20:balanceOf",
    block,
  });

  await sumMultiBalanceOf(balances, lockedPoSBalances);
  ...
  const lockedPlasmaBalances = await multiCall({
    calls: plasmaTokens,
    abi: "erc20:balanceOf",
    block,
  });
  ...
  const wrappedETHIndex = lockedPlasmaBalances.output.findIndex(
    v => v.success && v.input.target == "0xa45b966996374E9e65ab991C6FE4Bfce3a56DDe8",
  );
